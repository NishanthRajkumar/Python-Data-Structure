...
def sort_by_value(dictionary_to_sort: dict):
    """
        Description:
        ...
        Return:
            None
    """
    sorted_dictionary = {key: val for key, val in sorted(dictionary_to_sort.items(), key= lambda item: item[1])}
    logging.info(f"Orginal Dictionary: {dictionary_to_sort}")
    logging.info(f"Sorted by Value: {sorted_dictionary}")
