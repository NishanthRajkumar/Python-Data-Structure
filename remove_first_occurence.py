'''
    @Author: Nishanth
    @Date: 01-04-2022 21:30:00
    @Last Modified by: Nishanth
    @Last Modified Date: 01-04-2022 21:30:00
    @Title: Remove the first occurence of specififed item in array
'''
from array import array
import logging
import sys

def remove_first_occurence(homogenous_list: list, list_type: str, array_element):
    """
        Description:
            removes the first occurence of the element in array
        
        Parameter:
            homogenous_list: A list of items that are of same type
            list_type: type of element in list. accepted values: 'int', 'unicode', 'float'
            array_element: element in array to remove
    ...
    try:
        ...
        logging.info(f"Original Array: {array_from_list}")
        array_from_list.remove(array_element)
        logging.info(f"Updated Array: {array_from_list}")
    except TypeError:
        logging.exception(f"1 or more items in the list did not match the list type: {list_type}")
    except ValueError:
        logging.exception(f"{array_element} not found in array")
...
if __name__ == '__main__':
    ...
    remove_first_occurence([23, 4, 5, 7, 120, 4, 78, 4], 'int', 4)